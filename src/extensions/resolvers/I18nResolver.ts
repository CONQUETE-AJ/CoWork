 ...
 *
 * Example: `i18n/fr-FR/extension.json` with `{ "displayName": "Bonjour le monde" }`
 ...
 * Keys are locale codes (e.g. 'en-US', 'fr-FR'), values are flattened module objects.
 */
...
export async function loadExtensionLocales(ext: LoadedExtension): Promise<ExtensionLocaleData> {
  ...
  try {
    ...
    for (const entry of localeDirs) {
      ...
      const locale = entry.name; // e.g. 'en-US', 'fr-FR'
